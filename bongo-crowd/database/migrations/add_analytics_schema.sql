-- Migration: Add Analytics Schema for BONGO-CROWD
-- Created: 2026-06-22
...
CREATE INDEX IF NOT EXISTS idx_analytics_metrics_period ON analytics_metrics(period, period_start);

-- Materialized view: Platform Overview Statistics
DROP MATERIALIZED VIEW IF EXISTS mv_platform_stats;
...
DROP MATERIALIZED VIEW IF EXISTS mv_daily_submissions;
...
DROP MATERIALIZED VIEW IF EXISTS mv_monthly_analytics;
...
DROP MATERIALIZED VIEW IF EXISTS mv_vulnerability_types;
...
DROP MATERIALIZED VIEW IF EXISTS mv_researcher_leaderboard;
...
DROP MATERIALIZED VIEW IF EXISTS mv_response_time_metrics;
...
CREATE INDEX IF NOT EXISTS idx_reports_submitted_date ON reports(DATE(submitted_at));
CREATE INDEX IF NOT EXISTS idx_reports_bounty_paid ON reports(bounty_paid, bounty_paid_at);
CREATE INDEX IF NOT EXISTS idx_reports_severity_status ON reports(severity, status);

-- Insert initial data into materialized views
